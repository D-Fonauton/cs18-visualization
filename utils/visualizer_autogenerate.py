...
class Scanpath:
    ...
    def __call__(self, condition=None, object=None, subject=None, image=None):
        self.condition = None
        self.object = None
        self.subject = None
        self.image = None
        if all(var is not None for var in [self.condition, self.object, self.subject, self.image]):
            pass
...
class GazeVisualizer:
    def __init__(self, root: Tk, cfg: DictConfig):
        ...
        self.cfg = cfg
        self.dataset = COCOSearch18(cfg.dataset)
        self.point = Scanpath()
    ...
    def display_image_and_gaze(self):
        ...
        gaze = [g for g in gazes if (g["subject"] == SUBJECT_INDEX[self.current_subject])]
        ...
        if self.scanpaths_status.get():
            ...
            self.label.config(text=f"condition={self.condition},task object={self.current_object},image={img_name},subject={SUBJECT_INDEX[self.current_subject]}, correct: {correct}({correct_prob})")
            if self.animation_status.get():
                self.animate_gaze(image_path, gaze[0])
            else:
                self.plot_gazes(image_path, gaze)
        else:
            self.label.config(text=f"no scanpaths found with subject {SUBJECT_INDEX[self.current_subject]} searching for image: {img_name}")
    ...
    def previous_subject(self, event=None):
        if self.condition and self.current_object:
            self.current_subject = (self.current_subject - 1) % SUBJECT_NUMBER
            self.display_image_and_gaze()
